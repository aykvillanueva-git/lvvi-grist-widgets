...
Logic:
  ...
  - NEW: live-entry rows that fall inside the scope of the two named funds on the
    "Fund Monitoring (UB Tax + PHIC)" page also get a mirrored POSITIVE row in
    Fund_Remittance_Payments, so Fund_Accountability's payments_out (and therefore
    Net Fund Balance) updates automatically instead of Ayk re-entering the same
    total by hand from the Liquidation report:
      * Every live Tax_Remittances row -> Fund_Remittance_Payments, fund="UB Tax Fund"
      * Live Contribution_Remittances rows -> Fund_Remittance_Payments, fund="PHIC Fund"
        ONLY when contribution_type == "PHIC". SSS/HDMF/Blended live entries still get
        the Cash_Fund_Injections debit above (for Gladys's accountability) but are out
        of scope for this fund monitor, which only tracks UB Tax + PHIC, so no
        Fund_Remittance_Payments row is created for them.
...
def main():
    ...
    contrib_debits, contrib_fund_rows, contrib_grandfathered = process_table(
        ...
        lambda f: "PHIC Fund" if f.get("contribution_type") == "PHIC" else None,
    )
    ...
    print(f"Contribution_Remittances: {contrib_debits} posted as new Fund debits "
          f"({contrib_fund_rows} mirrored to Fund_Remittance_Payments, PHIC-only), "
